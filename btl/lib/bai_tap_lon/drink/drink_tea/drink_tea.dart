  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GetX<SP_Controller>(
        builder: (controller) {
          if (controller.dsspT.isEmpty) {
            return Center(child: CircularProgressIndicator());
          }
          return ListView.builder(
            itemCount: controller.dsspT.length,
            itemBuilder: (context, index) {
              var sp = controller.dsspT[index];
              return Card(
                ...
                child: GestureDetector(
                  onTap: () => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => PageChiTietDrinkTea(drinkTea: sp,),
                    ),
                  ),
                  child: Padding(
                    padding: const EdgeInsets.all(8.0),
...
class _SizeSelectionSheetState extends State<SizeSelectionSheet> {
  ...
  final List<String> _sizes = ["Nóng", "Đá"];
  ...
  void _generatePrices() {
    ...
    _prices = [
      basePrice,
      basePrice + 5000 + (random.nextInt(2) * 5000),
      basePrice + 2000 + (random.nextInt(2) * 5000),
    ];
  }
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              controller.themvaoGHDrTea(widget.sp, _quantity);
