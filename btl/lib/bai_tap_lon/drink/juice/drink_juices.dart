  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GetX<SP_Controller>(
        builder: (controller) {
          if (controller.dsspJ.isEmpty) {
            return Center(child: CircularProgressIndicator());
          }
          return ListView.builder(
            itemCount: controller.dsspJ.length,
            itemBuilder: (context, index) {
              var sp = controller.dsspJ[index];
              return Card(
                ...
                child: GestureDetector(
                  onTap: () => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => PageChiTietDrinkJuice(juices: sp,),
                    ),
                  ),
                  child: Padding(
                    padding: const EdgeInsets.all(8.0),
...
class _SizeSelectionSheetState extends State<SizeSelectionSheet> {
  ...
  final List<String> _sizes = ["Vừa", "To"];
  ...
  void _generatePrices() {
    ...
    _prices = [
      ...
      basePrice + 5000 + (random.nextInt(2) * 5000), // At least 10000 more than basePrice
    ];
  }
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              controller.themvaoJuice(widget.sp, _quantity);
